...
func (a *app) printCommandHelp(fs *flag.FlagSet) error {
	commands := map[string]struct{ args, description string }{
		...
		"omega publish": {"--home path --network id --manifest path --repository-dir path --version n --disposable", "Publish an approved three-root manifest and verify the exact release over HTTPS. Repeat the same version to recover an interrupted publication."},
