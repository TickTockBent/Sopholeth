...
func (a *app) printCommandHelp(fs *flag.FlagSet) error {
	commands := map[string]struct{ args, description string }{
		"omega init":              {"--home path --network id --repository https://host [--disposable [--encrypted]]", "Create an encrypted authority atomically. New passphrases require at least 12 characters. Repeating the command recovers or verifies the same authority. --disposable selects rehearsal custody."},
		"omega provision-renewal": {"--home operator-path --network id --renewal-home online-path [--disposable]", "Provision only snapshot/timestamp keys and hand off publication state to a separate home. Repeating a completed handoff needs no passphrase."},
		"omega publish":           {"--home path --network id [--disposable] [--renew | --manifest path --repository-dir path --version n]", "Approve a manifest with the operator's membership key, or use --renew from the operational home without a passphrase. Verify the exact release over HTTPS."},
		"omega status":            {"--home path --network id [--verify | --check-keys]", "Inspect public authority and release state without a passphrase. --verify checks HTTPS; --check-keys unlocks the active keys to verify a restored recovery set."},
		"omega rotate":            {"--home operator-path --network id --root-version n [--disposable] [--role online|targets|root] [--apply root-sha256 [--renew-approval]]", "Prepare a signed key transition, review it, then apply that exact digest. Root application also requires --renew-approval. Repeating the same version resumes the same transition."},
